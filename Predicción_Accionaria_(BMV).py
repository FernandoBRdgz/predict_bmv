...
stock_quote = raw_input('Ingrese el nombre de la acción en formato BMV que desea predecir (ej: CEMEXCPO, ELEKTRA, ALSEA): ').upper()
# Comprueba si el sentimiento de la acción es positivo
if not stock_sentiment(stock_quote, num_tweets=100):
    print 'La acción tiene sentimiento negativo, por favor corra de nuevo el código para otra acción'
    sys.exit()
# Verifica si existe o no el histórico
if not get_historical(stock_quote):
    print 'Google regresó un 404, por favor corra de nuevo el código'
